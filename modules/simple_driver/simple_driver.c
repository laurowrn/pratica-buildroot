...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
	int error_count = 0;
	struct message_data *node;
	size_t ret = 0;

	node = list_entry(head.next, struct message_data, member);
	ret = strlen(node->message) + 1;

	if(ret > len){
		ret = len;
	}

	error_count = copy_to_user(buffer, node->message, ret);
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
	...
	node = kmalloc(sizeof(struct message_data), GFP_KERNEL);

	if(len > 256){
		len = 256;
	}

	ret = copy_from_user(node->message, buffer, len);
	list_add_tail(&node->member, &head);

	if (len < sizeof(message)){             // store the length of the stored message
